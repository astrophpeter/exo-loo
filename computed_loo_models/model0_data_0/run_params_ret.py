...
NF=            [2000]#,1000]#,1299]#,1000]                                                # NF = number of wavelength values in each range if spacing is 'R' then the value is used as the resolution, options include: 'native' and 'passed' to give it a lam array. Define the directory of passed array in atm.profile
lam_sections = [0.3,2.0]#,5.1]#,3.5,4.6]#[0.77,1.87,5.5]#4.17,5.5][1.1,1.75]                # lam_sections = boundaries of wavelength ranges
spacing = "lam" #'lam' or 'R' for fixed resolution
instruments = ["stis1","stis2","wfc3"]               # instruments = which instruments are the data from?
pecul_name1 = "full_model_15"                                                                       # pecul_name_1 = label for the retrieval
num_live_pts = 1000

# Which functionalities do you want to use?
sampler='MultiNest'#MultiNest or PolyChord or Dynesty
retrieve =      True                                                                        #Are you running a retrieval?
resume= True
plot_spec =     True                                                                       #Do you want to plot the PT prof, spectrum, and corner plot?
...
leave_out_file_plot=False                                                                    #Are you plotting the LOO analyser?
...
true_values=[-3.3,-6.0,-7.0,1500.0,1.0,-4.0,0.0,-8.0,0.0]                                                                            #Do you have true values for the data you are retrieving? for instance something like [-5.0,-12.0,-12.0,-12.0,-12.0,-12.0,-12.0,1500,1.0,1.0,-2.0,-4.0,0.0,-0.3,0.0,-4.0,0.0,0.0]
...
isotherm=True                                                                               #Is the P-T profile an isotherm?
...
clouds_n_hazes=True                                                                         #If you retrieve a cloudy atmosphere, do you want to retrieve clouds AND hazes?
...
instruments_offset1=['stis1','stis2']
instruments_1_label='STIS'                                                                   #To appear like this in corner plot
...
mol_list = [                    "h2he",   "h2o",    "na",    "k"]#,    "alo",   "vo"]#,     "k"]#,    "ch4",   "nh3",   "hcn",   "co"]#
mol_lower_prior = np.array([     -10.0,   -15.0,   -15.0,   -15.0,   -12.0,   -12.0,   -12.0,    -12.0,    -12.0,   -12.0,    -12.0,  -12.0,   -12.0])
mol_upper_prior = np.array([       0.0,    -0.3,   -0.3,    -0.3,    -0.3,    -0.3,    -0.3,     -0.3,     -0.3,    -0.3,    -0.3,    -0.3,     -0.3])
mixing_frac = np.array([          5e-4,    -3.3,   -6.0,   -7.0,   -3.84,   -7.64,   -6.75,  -3.3,  -6.0]) #in log mixing frac
cia = ["h2", "he"]
...
if cloudy_atm:
    if clouds_n_hazes:
        priors["cl_loga"] = (-4.0,10.0); priors["cl_gamma"] = (-20.0,2.0); priors["cl_logPc"] = (-6.0,2.0); priors["cl_phi"] = (0.0,1.0)
        ret_params = ret_params + ["cl_loga","cl_gamma","cl_logPc"]
        partial_clouds = True
        if partial_clouds:
            ret_params.append("cl_phi")
    if clouds_only:
        priors["cl_logPc"] = (-7.0,3.0); priors["cl_phi"] = (0.0,1.0)
        ret_params = ret_params + ["cl_logPc"]
        partial_clouds = False
        if partial_clouds:
            ret_params.append("cl_phi")
...
default_params["T0"] = 1500.0
default_params["T100mb"] = 1725.0
default_params["alpha1"] = 0.97
default_params["alpha2"] = 1.09
default_params["P1"] = -1.69
default_params["P2"] = -4.12
default_params["P3"] = 0.56
default_params["Pref"] =1.0
default_params["lam_shift"] = 0.0
default_params["cl_loga"] = 6.50
default_params["cl_gamma"] = -6.40
default_params["cl_logPc"] = -4.19
default_params["cl_phi"] = 0.38
...
data_file_loc["wfc3"] = "data/"+planet+"_wfc3.txt" #Just to use the mandell data for WASP19b
data_file_loc["wfc3_102"] = "data/"+planet+"_wfc3_102.txt"
data_file_loc["stis1"] = "data/"+planet+"_stis1.txt"
data_file_loc["stis2"] = "data/"+planet+"_stis2.txt"
...
assert((clouds_n_hazes and clouds_only and cloudy_atm)==False), "Can't have clouds and hazes and clouds only simultanously selected"
